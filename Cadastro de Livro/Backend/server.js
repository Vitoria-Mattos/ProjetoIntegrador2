...
app.post('/livros/cadastrar', (req, res) => {
  const { id, titulo, autor, numero_paginas, categoria } = req.body;

  if (!id || !titulo || !autor || !numero_paginas || !categoria) {
    return res.status(400).json({ erro: 'Preencha todos os campos!' });
  }

  const sql = `INSERT INTO livros (id, titulo, autor, numero_paginas, categoria)
               VALUES (?, ?, ?, ?, ?)`;

  db.run(sql, [id, titulo, autor, numero_paginas, categoria], function (err) {
    if (err) {
